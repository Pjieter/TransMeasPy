# `TransMeasPy` developer documentation
...
# install TransMeasPy as an editable package
...
The second is to use `tox`, which can be installed separately (e.g. with `pip install tox`), i.e. not necessarily inside the virtual environment you use for installing `TransMeasPy`, but then builds the necessary virtual environments itself by simply running:
...
cd $(mktemp -d TransMeasPy.XXXXXX)
git clone git@github.com:Pjieter/TransMeasPy .
...
Visit
[https://test.pypi.org/project/TransMeasPy](https://test.pypi.org/project/TransMeasPy)
and verify that your package was uploaded successfully. Keep the terminal open, we'll need it later.

In a new terminal, without an activated virtual environment or an env directory:

```shell
cd $(mktemp -d TransMeasPy-test.XXXXXX)
...
--extra-index-url https://pypi.org/simple TransMeasPy
```
...
Don't forget to also make a [release on GitHub](https://github.com/Pjieter/TransMeasPy/releases/new).GitHub-Zenodo integration will also trigger Zenodo into making a snapshot of your repository and sticking a DOI on it.